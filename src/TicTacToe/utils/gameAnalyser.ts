function analyseGame(boardState: Array<Array<string>>): string {
    ...
    if(boardState[0][0] == boardState[1][1] && boardState[1][1]  == boardState[2][2] ) return `winner is ${boardState[0][0]}`
    if(boardState[0][2] == boardState[1][1] && boardState[1][1]  == boardState[2][0] ) return `winner is ${boardState[0][2]}`
